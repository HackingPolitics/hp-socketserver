  ...
  handleConnection(incoming: WebSocket, request: IncomingMessage, documentName: string, context: any = null): void {
    ...
    this.hooks('onConnect', hookPayload, (contextAdditions: any) => {
      ...
      .catch(error => {
        ...
        // @todo re-throw only truthy values to prevent UnhandledPromisRejections, if a hook/extension
        // throws errors, the error is handled in the hook-caller & false is thrown there
        if (error) {
          throw error
        }
      })
    ...
    for (let i = 0; i < extensions.length; i += 1) {
      chain = chain
        ...
        .catch(error => {
          ...
          if (error.message) {
            console.log(`[${name} hook]`, error.message)
          }

          // @todo throw a falsy value so our upper catch() can silently stop
      ...
      if (callback) {
        chain = chain
          ...
          .catch(error => {
            if (error.message) {
              console.log(`[${name} hook callback]`, error.message)
            }

            // throw a falsy value so our upper catch() can silently stop
            // eslint-disable-next-line no-throw-literal
            throw false
          })
      }
